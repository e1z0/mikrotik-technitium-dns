...
# - domain       forward DNS zone suffix used in FQDNs (example: lab01.efn.lt)
# - zone         Technitium forward zone (example: efn.lt)
# - dnsApiUrl    Technitium API base URL (example: https://dns.efn.lt/api/zones/records)
...
:local apiFetch do={
    ...
    :local body ""
    :do {
        :local result [/tool fetch url=$reqUrl as-value output=user]
        :set body ($result->"data")
    } on-error={
        :log warning ($LogPrefix . ": fetch failed for " . $opName)
        :set body ""
    }
    :return $body
}

# return: ok | benign | error
:local apiClassify do={
    :local body $1
    :local opMode $2
    :if ([:len $body] = 0) do={ :return "error" }
    :local lower [:convert $body transform=lc]
    :if ([:find $lower "\"status\":\"ok\""] != nil) do={ :return "ok" }
    :if (($opMode = "add") and ([:find $lower "already exists"] != nil)) do={ :return "benign" }
    :if (($opMode = "del") and ([:find $lower "not found"] != nil)) do={ :return "benign" }
    :return "error"
}
...
:local deleteA do={
    :local fqdn $1
    :local ipAddress $2
    :local url ($dnsApiUrl . "/delete?token=" . $apiKey . "&domain=" . $fqdn . "&zone=" . $zone . "&type=A&value=" . $ipAddress)
    $logDebug ("A del URL: " . $url)
    :local content [$apiFetch $url ("A delete " . $fqdn)]
    $logDebug ("A del BODY: " . $content)
    :local state [$apiClassify $content "del"]
    :if ($state = "benign") do={ $logDebug ("A delete benign for " . $fqdn . " -> " . $ipAddress) }
    :if ($state = "error") do={ :log warning ($LogPrefix . ": A delete API non-ok for " . $fqdn . " -> " . $ipAddress) }
}
...
:local deletePTR do={
    ...
    :local ptrZone $2
    :local fqdn $3
    :local url ($dnsApiUrl . "/delete?token=" . $apiKey . "&domain=" . $reverseFqdn . "&ptrName=" . $fqdn . "&zone=" . $ptrZone . "&type=ptr")
    $logDebug ("PTR del URL: " . $url)
    :local content [$apiFetch $url ("PTR delete " . $fqdn)]
    $logDebug ("PTR del BODY: " . $content)
    :local state [$apiClassify $content "del"]
    :if ($state = "benign") do={ $logDebug ("PTR delete benign for " . $reverseFqdn . " -> " . $fqdn) }
    :if ($state = "error") do={ :log warning ($LogPrefix . ": PTR delete API non-ok for " . $reverseFqdn . " -> " . $fqdn) }
}

:local addA do={
    :local fqdn $1
    :local ipAddress $2
    :local url ($dnsApiUrl . "/add?token=" . $apiKey . "&domain=" . $fqdn . "&zone=" . $zone . "&type=A&ipAddress=" . $ipAddress . "&overwrite=true&comments=" . $DHCPtag)
    $logDebug ("A add URL: " . $url)
    :local content [$apiFetch $url ("A add " . $fqdn)]
    $logDebug ("A add BODY: " . $content)
    :local state [$apiClassify $content "add"]
    :if ($state = "benign") do={ $logDebug ("A add benign for " . $fqdn . " -> " . $ipAddress) }
    :if ($state = "error") do={ :log warning ($LogPrefix . ": A add API non-ok for " . $fqdn . " -> " . $ipAddress) }
}
...
:local addPTR do={
    ...
    :local ptrZone $2
    :local fqdn $3
    :local url ($dnsApiUrl . "/add?token=" . $apiKey . "&domain=" . $reverseFqdn . "&ptrName=" . $fqdn . "&zone=" . $ptrZone . "&type=ptr&overwrite=true&comments=" . $DHCPtag)
    $logDebug ("PTR add URL: " . $url)
    :local content [$apiFetch $url ("PTR add " . $fqdn)]
    $logDebug ("PTR add BODY: " . $content)
    :local state [$apiClassify $content "add"]
    :if ($state = "benign") do={ $logDebug ("PTR add benign for " . $reverseFqdn . " -> " . $fqdn) }
    :if ($state = "error") do={ :log warning ($LogPrefix . ": PTR add API non-ok for " . $reverseFqdn . " -> " . $fqdn) }
}
